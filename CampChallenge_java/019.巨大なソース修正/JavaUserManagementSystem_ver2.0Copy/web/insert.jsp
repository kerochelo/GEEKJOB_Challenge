...
<html>
    ...
    <form action="insertconfirm" method="POST">
        名前:
        <input type="text" name="name" placeholder="例) 桃浦 金太郎" value="<% if(reinput){out.print(udb.getName());}%>">
        <br><br>
        ...
        電話番号(半角数字と,'-'で区切って記入):
        <br>
        <input type="text" name="tell" placeholder="例) 090-1234-5678" pattern="\d{2,4}-\d{3,4}-\d{3,4}" value="<% if(reinput){out.print(udb.getTell());}%>">
        <br><br>

        自己紹介文
        <br>
        <textarea name="comment" rows=10 cols=50 style="resize:none" wrap="hard" placeholder="例) hello java"><% if(reinput){out.print(udb.getComment());}%></textarea><br><br>
